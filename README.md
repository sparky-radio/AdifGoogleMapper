...
options:
  ...
  --wspr         the WSPR text file name and location
  --start START  the start date Y-M-D
  --end END      the end date Y-M-D
```
Either an ADIF file and location **or** a WSPR file and its location are required.  The date parameters are optional.  
...
## About the WSPR (ALL_WSPR.TXT) Format

*I am using the WSPR file created by WSJT-X as a standard for this program.*  
***Therefore fields following `Pwr` probably do not match the list below    
or WSPR logs generated by another source***  

### Field Descriptions

- `Date` - Date of the WSPR transmission/reception in YYMMDD format
- `Time` - Time of the WSPR transmission/reception in HHMM format (UTC)
- `SNR` - Signal-to-noise ratio in dB as measured by the receiving station
- `Drift` - Frequency drift in Hz/minute during the transmission
- `Freq` - Transmission frequency in MHz
- `TxCall` - Transmitting station's call sign
- `Grid` - Transmitting station's Maidenhead grid locator (4 or 6 characters)
- `Pwr` - Transmitter power in dBm (decibels relative to 1 milliwatt)
- `Reporter` - Call sign of the receiving/reporting station
- `RGrid` - Receiving station's Maidenhead grid locator
- `Distance` - Distance between transmitter and receiver (typically in km)
- `Direction` - Bearing/azimuth from receiver to transmitter in degrees
- `Code` - Error code field (added after Dec 22, 2010) - non-zero values indicate potentially erroneous spots (bogus callsign, wrong band, mixing products, etc.)
- `Version` - Version string of the WSPR software used by the receiving station (may be blank for older versions)

## Notes

- The Code field was added to archives generated after December 22, 2010
- Version reporting was only implemented starting with WSPR version 0.6 or 0.7
- All times are in UTC
- Power is reported in dBm (0 dBm = 1 mW, so 30 dBm = 1 W, 37 dBm = 5 W, etc.)
- Grid locators use the Maidenhead locator system
- Distance and direction calculations are based on the grid locators of both stations
